...
def pistonPlanePosition(xOfCentre,yOfCentre, panelHeight: float, baseToPanelRatio, maxPistonLength, theta_x, theta_y):
    ...
    if pistonLength1 > maxPistonLength or pistonLength2 > maxPistonLength or pistonLength3 > maxPistonLength:
        print('piston at maximum length!!!')
        return 0
    ...
    # for i in range(len(xOfPistons)):
    #     xOfPistons[i] = round(xOfPistons[i],2)
    #     yOfPistons[i] = round(yOfPistons[i],2)
    #     zOfPistons[i] = round(zOfPistons[i],2)
